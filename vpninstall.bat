...
if not exist "%TARGET_DIR%" (
    call :error "The directory %TARGET_DIR% does not exist. Please create it and try again."
    exit /b
)
...
call :info "Attempting to connect..."
for /f "tokens=*" %%A in ('curl -s -X POST %DOWNLOAD_API_URL% -H "Content-Type: application/json" -d "{ \"license\": \"%LICENSE_KEY%\", \"packages\": \"VPN\", \"resource_name\": \"VPN\" }"') do set RESPONSE=%%A
